...
def counter(data):
    ...
    for i in range(int(data['Num_of_inputs'])):
        inputs[data[f'{i}_index']] = float(data[f'{i}_input'])
        outputs[data[f'{i}_index']] = float(data[f'{i}_input'])
    prototypes = {}
    for i in range(int(data['Num_of_ribs'])):
        if data[f'{i}_in'] in prototypes.keys():
            prototypes[data[f'{i}_in']].append([data[f'{i}_out'], float(data[f'{i}_rib'])])
        else:
            prototypes[data[f'{i}_in']] = [[data[f'{i}_out'], float(data[f'{i}_rib'])]]
